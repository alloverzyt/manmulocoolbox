# 满木工具箱 · 开发交接文档 (DEVELOPMENT)

> 本文档面向开发者与 AI 助手（如 dsh），是项目的**唯一交接入口**。普通用户请看 [README.md](README.md)。

---

## 📌 文档维护规则（后续开发者必读）

> 任何人（人或 AI 助手）接手开发时，必须遵守以下规则，保证项目可持续演进。
...
| 新增插件 | ① 在 `src/plugins/<分类>/` 新建文件 ② 在 `config/plugin_config.json` 注册 ③ 更新 README.md「工具一览」表 ④ 在 `verify_all_plugins.py` 补充测试构造（如需特殊输入） |
| 新增外部依赖 | `src/core/dependency_manager.py` 添加下载源配置（先探测后选择机制），并在 CHANGELOG 说明 |
| 用户可见的功能变化 | 同步更新 README.md 的对应章节（安装/使用/FAQ） |
